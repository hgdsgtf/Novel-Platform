...
package object DBAPI {
  ...
  def startTransaction[A](
      block: PlanContext ?=> IO[A]
  )(using encoder: Encoder[A], ctx: PlanContext): IO[A] = {
    given newContext: PlanContext =
      ctx.copy(transactionLevel = ctx.transactionLevel + 1)
    ...
    def commitOrRollbackAction(result: Either[Throwable, A]): IO[A] =
      result match {
        ...
        /** 如果问题已经处理过了，我们不需要额外处理了 */
        case Left(exception) =>
          EndTransactionMessage(false).send >> IO.raiseError(
            DidRollbackException(exception)
          ) // 出现了问题，回滚
        case Right(value) =>
    ...
    for {
      _ <-
        startTransactionAction // Start the transaction if this is the first level
      result <- block(using
        newContext
      ).attempt // Execute the block with the new (incremented) transaction context
  ...
  def initSchema(
      schemaName: String
  )(using Encoder[InitSchemaMessage], PlanContext): IO[String] =
    InitSchemaMessage(schemaName).send

  def readDBRows(sqlQuery: String, parameters: List[SqlParameter])(using
      Encoder[ReadDBRowsMessage],
      PlanContext
  ): IO[List[Json]] =
    ReadDBRowsMessage(sqlQuery, parameters).send

  def readDBInt(sqlQuery: String, parameters: List[SqlParameter])(using
      context: PlanContext
  ): IO[Int] =
    for {
  ...
  def readDBString(sqlQuery: String, parameters: List[SqlParameter])(using
      context: PlanContext
  ): IO[String] =
    for {
      resultParam: String <- ReadDBValueMessage(sqlQuery, parameters).send
  ...
  def readDBBoolean(sqlQuery: String, parameters: List[SqlParameter])(using
      context: PlanContext
  ): IO[Boolean] =
    for {
      resultParam: String <- ReadDBValueMessage(sqlQuery, parameters).send
  ...
  def writeDB(sqlQuery: String, parameters: List[SqlParameter])(using
      Encoder[WriteDBMessage],
      PlanContext
  ): IO[String] = WriteDBMessage(sqlQuery, parameters).send

  def writeDBList(sqlQuery: String, parameters: List[ParameterList])(using
      Encoder[WriteDBListMessage],
      PlanContext
