...
object PortalService {
  def service(client: Client[IO]): HttpRoutes[IO] = HttpRoutes.of[IO] {
    ...
    case req @ POST -> Root / "api" / serviceName / messageName =>
      ...
      handlePostRequest(client, req, serviceName, messageName)
        .flatMap {
  ...
  def handlePostRequest(
    ...
    for {
      ...
      updatedJson = bodyJson.deepMerge(
        Json.obj("planContext" -> planContextJson)
      )
      _ <- IO.println(updatedJson)
      newUri <- IO.fromEither(
        Uri.fromString("http://" + address(serviceName) + "/api/" + messageName)
      )
      _ <- IO.println(newUri)
  ...
  def sendRequest(
    ...
    client
      ...
      .use { response =>
        ...
        if (response.status.isSuccess) {
          response
            ...
            .map(
              _.left.map(err =>
                new Exception(s"JSON parsing error: ${err.getMessage}", err)
              )
            )
        } else {
          response.bodyText.compile.string.map(bodyStr =>
            Left(
              new Exception(
                s"Received a non-success response: ${response.status}, Body: $bodyStr"
              )
            )
          )
        }
      ...
      .handleErrorWith(e =>
        IO.pure(
          Left(
            new Exception(s"Error during request execution: ${e.getMessage}", e)
          )
        )
      )
  }
  ...
  def internalServerError(e: Throwable): IO[Response[IO]] = {
    ...
    val errorMessage = Json.obj(
      "error" -> Json.fromString(
        s"${e.getMessage.replace(DidRollbackException.prefix, "")}"
      )
    )
    IO.pure(
