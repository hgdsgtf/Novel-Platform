      ...
      if (parts.length > 1 || combined.endsWith(delimiter))
        if (parts.length > 1)
          (
            parts.init.toList,
            parts.last
          ) // Return the last incomplete part to be combined with the next chunk
        else
        ...
        (
          List(),
          combined
        ) // Carry forward the combined data if no delimiter was found
    }.flatMap(acc => Stream.emits(acc._1).covary[F])

  def streamUpdates(client: Client[IO]): Stream[IO, Unit] = {
    val request =
      Request[IO](GET, uri"http://127.0.0.1:10008/stream/TongWen/tmp1")
