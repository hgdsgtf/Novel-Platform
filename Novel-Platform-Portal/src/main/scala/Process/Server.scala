...
object Server extends IOApp {
  ...
  def proxyStream(
    ...
    val targetUri =
      Uri.unsafeFromString(s"http://${address(serviceName)}/stream/$streamName")
      ...
      client
        ...
        .handleErrorWith { err =>
          if (attempt <= maxRetries) {
            ...
            IO.raiseError(
              new Exception(
                s"After $maxRetries attempts, failed to connect to $serviceName/$streamName: ${err.getMessage}",
                err
              )
            )
          }
        ...
        case None =>
          ...
          Topic[IO, Byte].flatMap { newTopic =>
            for {
              _ <- IO(
                topics
                  .putIfAbsent((serviceName, streamName), newTopic)
                  .getOrElse(newTopic)
              )
  ...
  override def run(args: List[String]): IO[ExitCode] = {
    Utils
      .readConfig(args.headOption.getOrElse("server_config.json"))
      .flatMap { config =>
        IO.println(
          s"Starting server on ${config.serverIP}:${config.serverPort}"
        ) *>
          (for {
            ...
            app <- Resource.eval(
              CORS.policy.withAllowOriginAll(httpApp(client))
            )
            ...
            server <- EmberServerBuilder
              ...
              .withHost(
                Host
                  .fromString(config.serverIP)
                  .getOrElse(
                    throw new IllegalArgumentException(
                      s"Invalid IPv4 address: ${config.serverIP}"
                    )
                  )
              )
              .withPort(
                Port
                  .fromInt(config.serverPort)
                  .getOrElse(
                    throw new IllegalArgumentException(
                      s"Invalid port: ${config.serverPort}"
                    )
                  )
              )
