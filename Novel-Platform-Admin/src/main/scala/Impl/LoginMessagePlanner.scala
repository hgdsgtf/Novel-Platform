...
import APIs.AdminAPI.LoginMessage
import cats.effect.IO
import io.circe.generic.auto.*
  ...
  override def plan(using planContext: PlanContext): IO[String] = {
    ...
    try {
      val checkUserExists = readDBBoolean(
        s"SELECT EXISTS(SELECT 1 FROM ${schemaName}.adminemail_adminuser_adminpassword_adminbook WHERE adminuser = ?)",
        List(SqlParameter("String", userName))
      )
      val checkHasPermission = readDBBoolean(
        s"SELECT EXISTS(SELECT 1 FROM ${schemaName}.adminemail_adminuser_adminpassword_adminbook WHERE adminuser = ? AND adminpassword = ?)",
