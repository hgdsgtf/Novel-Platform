...
const NovelComments= () => {
    ...
    const getCurrentStyleId = () => {
        try {
            const storedStyleId = localStorage.getItem(`currentStyleId_NovelComments`)
    ...
    useEffect(() => {
        fetchComments();
        fetchStyle();
    }, [novelTitle, currentStyleId]);
    const fetchStyle = async () => {
        try {
            const message = new GetPageStyleMessage('NovelComments', currentStyleId);
    ...
    const renderComments = (comments: Comment[], renderedComments: Set<number>, canDelete: boolean, parentId?: number) => {
        return comments
            ...
            .map((comment) => {
                ...
                return (
                    <div key={comment.id} className={parentId ? styles.commentReply : styles.commentRoot}>
                        <CommentItem
                            comment={comment}
    ...
    return (
        <div className={styles.commentContainer}>
            ...
            <div style={{ marginTop: '20px' }}>
                ...
                ></textarea>
                <button onClick={handleNewComment} className={styles.submitButton}>提交评论</button>
                <button
                    onClick={handleBack}
