...
import { ReaderSendCodeMessage } from 'Plugins/ReaderAPI/ReaderSendCodeMessage'
import { ReaderVerifyMessage } from 'Plugins/ReaderAPI/ReaderVerifyMessage'

const ReaderForgotPassword: React.FC = () => {
    ...
    const handleSendVerificationCode = async (email: string) => {
        ...
        try {
            const response = await axios.post(
                new ReaderSendCodeMessage(email).getURL(),
                JSON.stringify(new ReaderSendCodeMessage(email)),
                {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await axios.post(
                new ReaderVerifyMessage(email, verificationCode).getURL(),
                JSON.stringify(new ReaderVerifyMessage(email, verificationCode)),
                {
                    headers: { 'Content-Type': 'application/json' },
                }
            )
            ...
            setTimeout(() => {
                history.push(`/readerresetpassword?email=${email}`)
            }, 1000)
        } catch (err) {
    ...
    return (
        <form onSubmit={handleSubmit} style={{ textAlign: 'center', padding: '50px' }}>
            ...
            <div>
                <button
                    onClick={() => history.push('/readerlogin')}
