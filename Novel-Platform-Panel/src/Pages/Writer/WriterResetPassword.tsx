...
import { WriterResetMessage } from 'Plugins/WriterAPI/WriterResetMessage'
import { WriterGetUsernameMessage } from 'Plugins/WriterAPI/WriterGetUsernameMessage'

const WriterResetPassword: React.FC = () => {
    ...
    const FindUsernameByEmail = async () => {
        try {
            const response = await axios.post(
                new WriterGetUsernameMessage(email).getURL(),
                JSON.stringify(new WriterGetUsernameMessage(email)),
                {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await axios.post(
                new WriterResetMessage(name, password).getURL(),
                JSON.stringify(new WriterResetMessage(name, password)),
                {
                    headers: { 'Content-Type': 'application/json' },
                }
            )
