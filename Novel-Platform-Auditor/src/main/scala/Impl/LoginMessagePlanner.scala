...
import APIs.AuditorAPI.LoginMessage
import cats.effect.IO
import io.circe.generic.auto.*
  ...
  override def plan(using planContext: PlanContext): IO[String] = {
    try {
      ...
      val checkHasPermission = readDBBoolean(
        s"SELECT EXISTS(SELECT 1 FROM ${schemaName}.auditoremail_auditoruser_auditorpassword_auditorstatus WHERE auditoruser = ? AND auditorpassword = ?)",
