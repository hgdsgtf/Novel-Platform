...
case class GetQualifiedMessagePlanner(
    userName: String,
  ...
  override def plan(using PlanContext): IO[String] = {
    try {
      ...
      checkUserExists.flatMap { exists =>
        if (exists) {
          writeDB(
            s"UPDATE ${schemaName}.auditoremail_auditoruser_auditorpassword_auditorstatus SET auditorstatus = NOT auditorstatus WHERE auditoruser = ?",
            List(SqlParameter("String", userName))
          )
        } else {
