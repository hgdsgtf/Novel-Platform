...
case class SendCodeMessagePlanner(
  ...
  override def plan(using planContext: PlanContext): IO[String] = {
    ...
    val checkEmailExists = readDBBoolean(
      s"SELECT EXISTS(SELECT 1 FROM ${schemaName}.writeremail_writerverificationcode WHERE writeremail = ?)",
    ...
    checkEmailExists.flatMap { emailExists =>
      if (emailExists) {
        writeDB(
          s"UPDATE ${schemaName}.writeremail_writerverificationcode SET writerverificationcode = ? WHERE writeremail = ?",
          List(
        ...
        writeDB(
          s"INSERT INTO ${schemaName}.writeremail_writerverificationcode (writeremail, writerverificationcode) VALUES (?, ?)",
          List(
            SqlParameter("String", userEmail),
            SqlParameter("String", verificationCode)
          )
