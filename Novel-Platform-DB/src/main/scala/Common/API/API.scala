...
abstract class API[T: Decoder](targetService: String):
  ...
  def getURIWithAPIMessageName: IO[Uri] =
    for {
      ...
      newPath = baseUri.path.concat(
        Path.unsafeFromString(
          "/api/" + this.getClass.getSimpleName.stripPrefix("/")
        )
      )
      updatedUri = baseUri.withPath(newPath)
    } yield updatedUri

  def send(using Encoder[this.type], PlanContext): IO[T] =
...
object API {
  ...
  def send[T: Decoder, A <: API[T]: Encoder](
      message: A
  )(using context: PlanContext): IO[T] =
    for {
      ...
      result <- client.get.run(request).use { response =>
        val handler =
          summon[ResponseHandler[
            T
          ]] // Summon an instance of ResponseHandler for T
          ...
          case _ =>
            response.bodyText.compile.string.flatMap { body =>
              (if (body.startsWith(DidRollbackException.prefix))
                 IO.raiseError(
                   DidRollbackException(
                     new Exception(
                       s"${body.substring(DidRollbackException.prefix.length)}"
                     )
                   )
                 )
               else
