...
case class ReadDBValueMessagePlanner(
    sqlQuery: String,
  ...
  override def planWithConnection(
    ...
    try {
      ...
      parameters.zipWithIndex.foreach { case (param, index) =>
        param.dataType.toLowerCase match {
          ...
          case "datetime" =>
            ...
            ) // Convert DateTime to Timestamp          // Add more type cases as needed
      ...
      if (resultSet.next()) {
        resultSet.getString(
          1
        ) // Assuming the value you want is in the first column
      } else {
        throw new NoSuchElementException(
          "No value found for the given query and parameters."
        )
      }
    } finally {
      preparedStatement
