...
object JsonStringEncoder {
  ...
  implicit val stringEncoder: JsonStringEncoder[String] = (value: String) =>
    value

  // For other types that can be encoded as JSON using Circe
  implicit def circeJsonEncoder[T](implicit
      encoder: Encoder[T]
  ): JsonStringEncoder[T] = (value: T) => value.asJson.noSpaces
}
