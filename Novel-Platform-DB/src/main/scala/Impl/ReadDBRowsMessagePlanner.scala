...
case class ReadDBRowsMessagePlanner(
    sqlQuery: String,
    parameters: List[SqlParameter],
    override val planContext: PlanContext
) extends DBPlanner[List[Json]] {
  override def planWithConnection(
      connection: Connection,
      connectionMap: Ref[IO, Map[String, Connection]]
  ): IO[List[Json]] = IO.delay {
    ...
    try {
      ...
      parameters.zipWithIndex.foreach { case (param, index) =>
        param.dataType.toLowerCase match {
          ...
          case "boolean" =>
            preparedStatement.setBoolean(index + 1, param.value.toBoolean)
          case "datetime" =>
            preparedStatement.setTimestamp(
              index + 1,
              new Timestamp(param.value.toLong)
      ...
      while (resultSet.next()) {
        val rowJson = (1 to columns).foldLeft(Json.obj()) {
          (jsonObj, columnIndex) =>
            ...
            val processedValue: Json = value match {
              case null if columnType.startsWith("_") =>
                Json.arr() // Handle null for array types as empty array
              case array: PgArray =>
                Json.arr(array.getArray.asInstanceOf[Array[_]].map {
              ...
              case s: String if columnName.endsWith("_") =>
                parser.parse(s).fold(throw _, a => a)
              case s: String                => Json.fromString(s)
      ...
      preparedStatement
        .close() // Ensure the PreparedStatement is closed after use
    }
  }
}
