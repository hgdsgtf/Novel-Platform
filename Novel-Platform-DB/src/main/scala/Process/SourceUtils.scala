...
object SourceUtils {
  ...
  def initDB(connection: Connection, dbName: String): IO[Unit] = {
    ...
    IO {
      val dbExistsQuery =
        s"SELECT 1 FROM pg_database WHERE datname = ?" // Use prepared statements to prevent SQL injection
      ...
      try {
        ...
        if (!dbExistsResult.next()) {
          ...
          try {
            ...
            createDbStatement
              .close() // Ensure statement is closed after execution
          }
        }
      } finally {
        dbExistsStatement
          .close() // Ensure prepared statement is closed after execution
      }
    }
  }
}
