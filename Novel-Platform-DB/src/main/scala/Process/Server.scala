...
object Server extends IOApp {
  ...
  override def run(args: List[String]): IO[ExitCode] =
    Utils
      .readConfig(args.headOption.getOrElse("default_config.json"))
      .flatMap { defaultConfig =>
        createDataSource(defaultConfig).use { dataSource =>
          ...
          Resource
            .make(IO(dataSource.getConnection))(conn => IO(conn.close()))
            .use { connection =>
              for {
                ...
                server <- BlazeServerBuilder[IO]
                  ...
                  .withHttpApp(
                    Router(
                      "/" -> Routes.service(dataSource, connectionMap)
                    ).orNotFound
                  )
                  ...
                  .withResponseHeaderTimeout(
                    5.minutes
                  ) // Note: Ember's 'requestHeaderReceiveTimeout' may correspond to Blaze's 'responseHeaderTimeout' or similar.
                  .serve
                  .compile
                  .drain
                  .as(ExitCode.Success)
      ...
      .handleErrorWith { e =>
        IO(println(s"Server error: ${e.getMessage}"))
          .as(ExitCode.Error) // Log the error and return an error exit code
      }
}
